...
export function AppDataProvider({ children }) {
  ...
  async function addSeasonToTeam(teamId, season) {
    const item = {
      id: uuidv4(),
      name: season.name,
      part: season.part || "veld_najaar",
      opponents: [],
    };
    if (useFirestore) {
      try {
        ...
        const snap = await getDoc(ref);
        const data = snap.data() || {};
        await updateDoc(ref, { seasons: [...(data.seasons || []), item] });
        return item.id;
      } catch (e) {
        console.error("[Firestore] Fout bij seizoen toevoegen:", e);
        return null;
      }
    }
    setTeams(prev =>
      prev.map(t =>
        t.id === teamId ? { ...t, seasons: [...(t.seasons || []), item] } : t
      )
    );
    return item.id;
  }

  async function updateSeasonForTeam(teamId, seasonId, updates) {
    if (useFirestore) {
      try {
        const { doc, getDoc, updateDoc } = await ensureFirebaseFns();
        const ref = doc(db, "teams", teamId);
        const snap = await getDoc(ref);
        const data = snap.data() || {};
        const nextSeasons = (data.seasons || []).map((s) =>
          s.id === seasonId ? { ...s, ...updates } : s
        );
        await updateDoc(ref, { seasons: nextSeasons });
        return true;
      } catch (e) {
        console.error("[Firestore] Fout bij seizoen bijwerken:", e);
        return false;
      }
    }
    setTeams(prev =>
      prev.map(t =>
        t.id === teamId
          ? { ...t, seasons: (t.seasons || []).map((s) => s.id === seasonId ? { ...s, ...updates } : s) }
          : t
      )
    );
    return true;
  ...
  async function deleteSeasonFromTeam(teamId, seasonId) {
    if (useFirestore) {
      try {
        const { doc, getDoc, updateDoc } = await ensureFirebaseFns();
        const ref = doc(db, "teams", teamId);
        const snap = await getDoc(ref);
        const data = snap.data() || {};
        await updateDoc(ref, { seasons: (data.seasons || []).filter((s) => s.id !== seasonId) });
        return true;
      } catch (e) {
        console.error("[Firestore] Fout bij seizoen verwijderen:", e);
        return false;
      }
    }
    setTeams(prev =>
      prev.map(t =>
        t.id === teamId ? { ...t, seasons: (t.seasons || []).filter((s) => s.id !== seasonId) } : t
      )
    );
    return true;
  }

  async function addOpponentToTeam(teamId, opponentName, seasonId = null) {
    if (useFirestore) {
      try {
        const { doc, getDoc, updateDoc } = await ensureFirebaseFns();
        const ref = doc(db, "teams", teamId);
        ...
        if (seasonId) {
          const nextSeasons = (data.seasons || []).map((s) =>
            s.id === seasonId ? { ...s, opponents: [...(s.opponents || []), opponentName] } : s
          );
          await updateDoc(ref, { seasons: nextSeasons });
        } else {
      ...
      prev.map(t =>
        t.id === teamId
          ? seasonId
            ...
            : { ...t, opponents: [...(t.opponents || []), opponentName] }
          : t
      )
    );
  }

  async function removeOpponentFromTeam(teamId, opponentName, seasonId = null) {
    if (useFirestore) {
      try {
        const { doc, getDoc, updateDoc } = await ensureFirebaseFns();
        const ref = doc(db, "teams", teamId);
        ...
        if (seasonId) {
          const nextSeasons = (data.seasons || []).map((s) =>
            s.id === seasonId ? { ...s, opponents: (s.opponents || []).filter(o => o !== opponentName) } : s
          );
          await updateDoc(ref, { seasons: nextSeasons });
        } else {
